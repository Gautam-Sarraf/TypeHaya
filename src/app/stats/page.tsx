...
export default function StatsPage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      // Compute from guest localStorage
      computeGuestStats();
    }
  }, [user]);
    ...
    >
      ...
      <div className="flex-1 w-full max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
              <BarChart2 className="w-6 h-6" style={{ color: "var(--main-color)" }} />
              {user ? `${user.username}'s Statistics` : "Guest Statistics"}
            </h1>
            <p className="text-xs font-mono opacity-60 mt-1" style={{ color: "var(--sub-color)" }}>
              Detailed overview of your typing speed, accuracy, and all-time records.
            </p>
          </div>
          ...
          {!user && (
            <Link
              href="/login"
              className="px-4 py-2 rounded-xl text-xs font-bold shadow-md flex items-center gap-1.5 transition-transform hover:scale-105"
              style={{
                backgroundColor: "var(--sub-alt-color)",
                color: "var(--main-color)",
              }}
            >
              <span>Sign in to backup your stats</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div
            className="p-5 rounded-2xl flex flex-col gap-1 shadow-xs"
            style={{ backgroundColor: "var(--sub-alt-color)" }}
          >
            <span className="text-[11px] font-bold uppercase tracking-wider opacity-60 flex items-center gap-1.5" style={{ color: "var(--sub-color)" }}>
              <Flame className="w-3.5 h-3.5" style={{ color: "var(--main-color)" }} />
              Highest Speed
            </span>
            <span className="text-3xl font-extrabold" style={{ color: "var(--main-color)" }}>
              {stats?.highestWpm || 0} <span className="text-xs font-normal opacity-70">WPM</span>
            </span>
            <span className="text-[10px] opacity-40">Personal peak speed</span>
          </div>
          ...
          >
            ...
            <span className="text-3xl font-extrabold" style={{ color: "var(--text-color)" }}>
              {stats?.avgWpm || 0} <span className="text-xs font-normal opacity-70">WPM</span>
            </span>
            <span className="text-[10px] opacity-40">Across all completed tests</span>
          </div>
          ...
          >
            ...
            <span className="text-[10px] opacity-40">Typing precision</span>
          </div>
          ...
          >
            ...
            <span className="text-[10px] opacity-40">{stats?.totalTests || 0} tests taken</span>
          </div>
        </div>

        {/* Speed Progress Chart */}
        <div
          className="w-full p-6 rounded-2xl shadow-xs mb-8 flex flex-col gap-4"
          style={{ backgroundColor: "var(--sub-alt-color)" }}
        >
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold flex items-center gap-2">
              <TrendingUp className="w-4 h-4" style={{ color: "var(--main-color)" }} />
              Recent Speed Progression
            </h2>
            <span className="text-xs font-mono opacity-50" style={{ color: "var(--sub-color)" }}>
              Last 15 Tests
            </span>
          </div>
                  ...
                  <Tooltip
                    content={({ active, payload }) => {
                      if (active && payload && payload.length) {
                        ...
                        return (
                          <div
                            className="px-3 py-1.5 rounded-lg text-xs font-mono shadow-md border"
                            style={{
                          ...
                          >
                            <p style={{ color: "var(--main-color)" }}>{data.wpm} WPM</p>
        ...
        <div
          className="w-full rounded-2xl overflow-hidden shadow-xs border"
          style={{
            backgroundColor: "var(--sub-alt-color)",
            borderColor: "transparent",
          }}
        >
          <div className="p-4 border-b flex items-center gap-2" style={{ borderColor: "var(--bg-color)" }}>
            <Trophy className="w-4 h-4" style={{ color: "var(--main-color)" }} />
            <h2 className="text-sm font-bold">Personal Bests</h2>
          </div>
            ...
            <table className="w-full text-left font-mono text-xs">
              <thead>
                <tr className="border-b" style={{ borderColor: "var(--bg-color)", color: "var(--sub-color)" }}>
                  <th className="py-3 px-6 font-bold uppercase text-[10px]">Mode</th>
                  <th className="py-3 px-6 font-bold uppercase text-[10px]">Sub-Mode</th>
                  <th className="py-3 px-6 font-bold uppercase text-[10px] text-right">Best WPM</th>
                  <th className="py-3 px-6 font-bold uppercase text-[10px] text-right">Raw WPM</th>
                  <th className="py-3 px-6 font-bold uppercase text-[10px] text-right">Accuracy</th>
                </tr>
              </thead>
                ...
                {stats?.personalBests && stats.personalBests.length > 0 ? (
                  stats.personalBests.map((pb, idx) => (
                    <tr key={idx} className="hover:bg-black/10">
                      ...
                      <td className="py-3.5 px-6 text-right font-extrabold text-sm" style={{ color: "var(--main-color)" }}>
                        {pb.wpm}
                      </td>
